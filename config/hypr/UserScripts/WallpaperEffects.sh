...
no-effects() {
  SWWW_FPS="$FPS" SWWW_TYPE="$TYPE" SWWW_DURATION="$DURATION" SWWW_BEZIER="$BEZIER" \
    "$SCRIPTSDIR/WallpaperApply.sh" apply "$focused_monitor" "$wallpaper_current"
...
main() {
    ...
    choice=$(printf "%s\n" "${options[@]}" | LC_COLLATE=C sort | rofi_menu_pick "Efeitos" "$rofi_theme")
    ...
    if [[ -n "$choice" ]]; then
        if [[ "$choice" == "No Effects" ]]; then
            ...
            SWWW_FPS="$FPS" SWWW_TYPE="$TYPE" SWWW_DURATION="$DURATION" SWWW_BEZIER="$BEZIER" \
              "$SCRIPTSDIR/WallpaperApply.sh" apply "$focused_monitor" "$wallpaper_output"
...
# Encerra launcher anterior, se houver
rofi_prepare
