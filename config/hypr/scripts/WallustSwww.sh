...
# Recarrega a waybar só se o tema ativo usa cores do wallust.
waybar_uses_wallust() {
  local style
  style=$(readlink -f "$HOME/.config/waybar/style.css" 2>/dev/null || true)
  [[ -n "$style" && -f "$style" ]] && grep -q 'wallust/colors-waybar' "$style"
}

reload_waybar_if_needed() {
  [[ "${SKIP_WAYBAR_RELOAD:-}" == "1" ]] && return 0
  waybar_uses_wallust || return 0
