#!/usr/bin/env bash
# Worker root: encerra sessão Hyprland e troca para TTY do greeter SDDM.

sddm_greeter_vt() {
  local vt="" pid args

  if command -v journalctl >/dev/null; then
    vt=$(journalctl -b -u sddm --no-pager 2>/dev/null \
      | sed -n 's/.*Using VT \([0-9]\+\).*/\1/p' | tail -1)
  fi
  [[ -n "$vt" ]] && { echo "$vt"; return; }
